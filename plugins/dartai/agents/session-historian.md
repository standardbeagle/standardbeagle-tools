...
- **Read only the paths the orchestrator gave you.** Use `Read` on each `path`. Do not read source session files directly under `~/.claude/projects/`, `~/.codex/sessions/`, or `~/.cursor/projects/` — those are MB-scale and would blow the context window. The orchestrator already extracted what's relevant.
...
- Use `Read` for each path the orchestrator supplied. Do not pipe `cat` through shell — `Read` avoids permission prompts and is more reliable.
- `Grep` is appropriate when you want to locate a specific keyword across the supplied scratch files (not across source session files).

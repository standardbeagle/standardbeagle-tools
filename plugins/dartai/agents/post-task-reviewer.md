...
This agent emits **verdict-only** output per the canonical schema in `plugins/dartai/skills/verdict-schema.md`. Delivery is via a **verdict file** at `.dartai/reports/<task-id>/security.md`; stdout body is ≤5 lines (path pointer + one-line verdict). Internal phases below shape *how this agent thinks*; the loop driver consumes only the verdict file. Replan recommendations and security depth go into the verdict file's `evidence:` line (a written report). See "Return Format" section for the file format and stdout contract.
...
Do NOT inline the YAML block in stdout when using the file channel. Do NOT prose-narrate findings in stdout. The main loop parses the file; the transcript is dropped. Operators read the `evidence:` reference when reviewing depth.

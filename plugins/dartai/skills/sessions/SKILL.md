---
name: sessions
description: "Search + synthesize prior agent session history. Claude Code primary (~/.claude/projects/); Codex/Cursor secondary. Discover -> branch/keyword filter -> extract skeleton/errors -> session-historian subagent. Use when: what was tried, prior attempts, past decisions, recent work, why approach abandoned, cross-session context, even without saying 'sessions'."
disable-model-invocation: true
---

# /sessions

Search session history (Claude Code primary; Codex + Cursor secondary) and synthesize findings about what was worked on, tried, decided, or learned.

## Usage

```
/sessions [question or topic]
/sessions
```
...
If the line above resolved to a plain branch name (like `feat/my-branch`), use it for branch filtering and pass it to the synthesis subagent. If empty, derive at runtime.
...
- **Claude Code (primary)** — `~/.claude/projects/-<encoded-cwd>/<session-id>.jsonl`. Carries `gitBranch`, timestamps, structured user/assistant/tool blocks. The richest source.
- **Codex (secondary)** — `~/.codex/sessions/**/*.jsonl` or `~/.agents/sessions/**/*.jsonl`. No `gitBranch`; carries `cwd` instead. Discovery returns sessions across all repos — the cwd-filter prunes other repos before keyword scanning.
- **Cursor (secondary)** — `~/.cursor/projects/<repo>/agent-transcripts/*.jsonl`. No timestamps in JSONL (uses file mtime); no tool-result logging (errors-mode produces nothing).

The discovery script handles all three. Default to all platforms; restrict via `--platform claude|codex|cursor` only when the user is asking about one specific tool.
...
If no question argument is provided, ask the user what they want to know about their session history using `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Never silently skip the question.
...
```bash
SCRATCH=$(mktemp -d -t sessions-XXXXXX)
```
...
Dispatch the `session-historian` subagent via the `Agent` tool. Omit the `mode` parameter so the user's configured permission settings apply. Run on the mid-tier model (`model: "sonnet"`) — the synthesizer doesn't need frontier reasoning.
...
1. /tmp/sessions-XXXX/abc123.skeleton.txt
   platform=claude branch=feat/auth-fix ts=2026-05-01
2. /tmp/sessions-XXXX/def456.skeleton.txt  errors=/tmp/sessions-XXXX/def456.errors.txt
...
The agent reads each path via `Read` and returns prose findings. Bulk extraction content lives only in the agent's subagent context — the orchestrator's working state stays at file paths plus small inventory metadata.
...
When the caller (typically a user typing `/sessions`, or another skill invoking sessions via the platform's skill-invocation primitive) does not specify an output format, include a brief header noting what was searched:

...
Image processing MCP server: lossy/lossless optimization and format conversion (PNG/JPG/WebP/AVIF), responsive image variants with srcset generation, BlurHash and LQIP placeholders for lazy loading, SVG optimization (SVGO), and dominant-color palette extraction with percentage breakdown — a complete image pipeline for performant web delivery.
...
/slop-mcp:slop-add image-processing npx -y @standardbeagle/image-processing@latest mcp
```
...
```bash
cp plugins/image-processing/mcp.json.example plugins/image-processing/mcp.json
```

**Caveat**: this conflicts with slop-mcp. If both are active, you will see duplicate `image-processing` tools. Pick exactly one path per workspace. The project memory note at the marketplace root (`MEMORY.md` → *MCP Configuration*) records this trade-off and should not be invalidated by enabling `mcp.json` here.

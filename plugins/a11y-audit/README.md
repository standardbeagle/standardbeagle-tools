...
Static accessibility audit MCP server: HTML audit against WCAG 2.1/2.2 rules, ARIA role/property validation, heading outline analysis, link text quality checks, and tagged PDF accessibility inspection — all without a browser.
...
/slop-mcp:slop-add a11y-audit npx -y @standardbeagle/a11y-audit@latest mcp
```
...
```bash
cp plugins/a11y-audit/mcp.json.example plugins/a11y-audit/mcp.json
```

**Caveat**: this conflicts with slop-mcp. If both are active, you will see duplicate `a11y-audit` tools. Pick exactly one path per workspace. The project memory note at the marketplace root (`MEMORY.md` → *MCP Configuration*) records this trade-off and should not be invalidated by enabling `mcp.json` here.

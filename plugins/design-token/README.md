...
Design token MCP server: generate tokens, export to CSS/Tailwind/Style Dictionary/SCSS/JSON, validate schemas, diff token sets across versions, merge theme layers, and generate Tailwind configs — a Style Dictionary-compatible pipeline for design system token management.
...
/slop-mcp:slop-add design-token npx -y @standardbeagle/design-token@latest mcp
```
...
```bash
cp plugins/design-token/mcp.json.example plugins/design-token/mcp.json
```

**Caveat**: this conflicts with slop-mcp. If both are active, you will see duplicate `design-token` tools. Pick exactly one path per workspace. The project memory note at the marketplace root (`MEMORY.md` → *MCP Configuration*) records this trade-off and should not be invalidated by enabling `mcp.json` here.

...
description: "架構/安全/性能聲明須引≥2獨立源 or escalate gap to user. Load-bearing claim = 錯則下游連鎖 — wrong session-store breaks every middleware; bcrypt vs argon2 sets credential baseline; O(log n) vs O(n) breaks consumers at scale. Silent single-source = canonical rationalization-trap shape per K2 §3.3, links to knowledge-hygiene rationalization-trap-check (commit b28aa0f). Reuses multi-source-research pipeline. Use when: writing architectural decision in spec/design doc/ADR, asserting security primitive selection in code/review, asserting performance characteristic downstream consumers depend on, reviewing PR introducing load-bearing assertion, dispatching subagent producing design doc/ADR. Skip: non-load-bearing claim (typo, comment, docstring), single authoritative source explicitly documented (e.g., function signature via lci_get_context — source IS the artifact), exploratory claim labeled hypothesis."
---
